...
const TeacherHeader = () => {
  ...
  return (
    userData && (
      <div className="h-16 sm:h-20 md:h-24 w-full bg-blue-600 flex items-center px-4 sm:px-6 md:px-8">
        <div className="flex justify-between items-center w-full">
          <h1 className="text-white font-bold text-xl sm:text-2xl md:text-3xl">
            QuizMaster-2K25 |{" "}
            <span className="text-sm sm:text-lg md:text-xl">Faculty Panel</span>
          </h1>
          <div className="relative">
            ...
            >
              ...
              <button
                className="hidden md:flex w-full md:w-24 h-8 md:h-8 text-blue-600 md:bg-white md:hover:bg-red-600 md:hover:text-white  items-center justify-center font-bold rounded-lg text-sm sm:text-base md:rounded-lg bg-red-600 mx-2 md:mx-0"
                onClick={async () => {
                  await supabase.auth.signOut();
                  setLoggedIn(false);
                  setUserData(null);
